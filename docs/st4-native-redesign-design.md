...
- tags: `p h1-h6 a strong em b i code pre ul ol li blockquote img br hr span div`
  (the compatibility fixture decided against `table tr td th`: minihtml does not
  implement them, so `renderer/tables.py` rewrites every table into padded
  monospace rows before serialisation — see
  [ADR 0007](adr/0007-table-rendering-under-minihtml.md))
